...
interface EventCardProps {
  ...
  index?: number // ✅ Added for unique background
}

const EventCard: React.FC<EventCardProps> = ({ event, onEdit, onDelete, index = 0 }) => {
  // Define background color variants
  const bgVariants = [
    'bg-purple-100',
    'bg-blue-100',
    'bg-green-100',
  ...
  return (
    <Card
      className={cn(
        bgColor,
        'transition-all duration-300 h-full flex flex-col  hover:shadow-lg hover:scale-[1.02] shadow'
      )}
    >
      <CardHeader className='space-y-1 pb-2 flex flex-col items-center'>
        <CardTitle className='text-xl font-bold text-center'>{event.title}</CardTitle>
